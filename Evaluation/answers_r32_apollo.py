...
with open('generated_output_evaluation_r32_temp0.3.txt', 'w') as f:
    for i in range(len(test)):
        ...
        prompt = (
            f'''<|system|>:\n {instructions_string} <|prompter|>: {comment} 
...
df.to_csv('unbalanced_test_generated_output_evaluation_Apollor32.csv', index=False)
...
plt.title('Apollo 2B confusion matrix')
plt.savefig('Apollo2B_confusion_matrix_r32_whole_test.png', dpi=300, bbox_inches='tight')
# plt.show
